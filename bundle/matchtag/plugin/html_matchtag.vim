...
fu! s:highlightpair()
	...
	cal s:highlight(pos[0], pos[0] == [0, 0] && pos[1] ? 'Error' : 'MatchParen')
endf
...
fu! s:searchpair(tagn, forw)
	...
	let timeout  = 300
	let doerror  = index(s:selfclosing, a:tagn) >= 0 ? 0 : 1
...
aug MatchHtmlTag
	au!
	au CursorMoved,CursorMovedI,WinEnter <buffer> cal s:highlightpair()
aug END